...
#define WATER_PRE_COMPUTE
    ...
    {
        float d1 = DecodeFloatRG(height.rg);
        float d2 = DecodeFloatRG(height.ba);
        if (d1 >= d2)
            return d1;
        else 
            //负高度
    ...
    float3 GerstnerWave(
    float4 wave,float3 position,inout float3 tangent,inout float3 binormal,float waveSpeed,float scale)
    {
        //陡峭度
