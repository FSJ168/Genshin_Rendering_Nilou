...
// FCharacterDissolveOutput CalculateCharacterDissolve(FCharacterDissolveInput Input){

//     FCharacterDissolveOutput Output;
//     Output.DissolveAlpha=1.0;
//     Output.CurveTime=1.0;
//     Output.EdgeBlurFactor=0.0;
    
//     float DistanceToCharacter=distance(Input.CameraPositionWS,Input.ObjectPositionWS);
//     Output.CurveTime=saturate(DistanceToCharacter/CHARACTER_DISSOLVE_DISTANCE);

//     float2 ScreenUV=Input.ScreenPosition.xy/Input.ScreenPosition.w;
//     ScreenUV=ScreenUV*_ScreenParams.xy;

//     float2 UVOffset=float2(Output.CurveTime*0.1,Output.CurveTime*0.1);
//     float2 FinalUV=ScreenUV+UVOffset;
//     float NoiseAlpha=SAMPLE_TEXTURE2D(_GridTex,sampler_GridTex,FinalUV).a;
//     float HardEdgeMask=step(NoiseAlpha,Output.CurveTime);
//     Output.EdgeBlurFactor=smoothstep(Output.CurveTime-EDGE_BLUR_AMOUNT,Output.CurveTime+EDGE_BLUR_AMOUNT,NoiseAlpha);
//     Output.DissolveAlpha=saturate(HardEdgeMask*Output.EdgeBlurFactor);
...
half4 GenshinStyleFragment(CoreVaryings input,FRONT_FACE_TYPE isFrontFace:FRONT_FACE_SEMANTIC):SV_Target{
    ...
    // //Alpha Clip
    // FCharacterDissolveInput ClipInput;
    // ClipInput.CameraPositionWS=_WorldSpaceCameraPos.xyz;
    // ClipInput.ObjectPositionWS=_ObjectCenterWS.xyz;
    // ClipInput.ScreenPosition=input.positionCS;
    // FCharacterDissolveOutput Output=CalculateCharacterDissolve(ClipInput);
